...
static void usbd_work_handler(struct k_work *item)
{
    ...
    SYS_LOG_DBGX("[%08X] /work/",(uint32_t) k_current_get());
	...
	while ((ev = usbd_evt_get()) != NULL) {
		...
		switch(ev->evt)
		{
		    case EP_EVT_SETUP_RECV: {
                ...
                // For compatibility with the USB stack, SETUP packet must be reassembled.
		    ...
		    case EP_EVT_RECV_COMPLETE:
                SYS_LOG_DBGX("EP_EVT_RECV_COMPLETE @ %d",ep_ctx->cfg.addr);
...
static void usbd_event_transfer_ctrl(nrfx_usbd_evt_t const * const p_event)
{
    ...
    if ( NRF_USBD_EPIN_CHECK(p_event->data.eptransfer.ep) ) {
        SYS_LOG_DBGX("EPTRANSFER CTRL IN %d / %d", p_event->data.eptransfer.ep, p_event->data.eptransfer.status);
        ...
        SYS_LOG_DBGX("EPTRANSFER CTRL OUT %d / %d", p_event->data.eptransfer.ep, p_event->data.eptransfer.status);
...
static void usbd_event_transfer_data(nrfx_usbd_evt_t const * const p_event)
{
    ...
    if ( NRF_USBD_EPIN_CHECK(p_event->data.eptransfer.ep) ) {
        SYS_LOG_DBGX("EPTRANSFER IN %d / %d", p_event->data.eptransfer.ep, p_event->data.eptransfer.status);
        if (p_event->data.eptransfer.status == NRF_USBD_EP_OK) {
            struct usbd_ep_event *ev = usbd_evt_alloc();
            ev->ep = ep_ctx;
            ev->evt = EP_EVT_WRITE_COMPLETE;
        ...
        SYS_LOG_DBGX("EPTRANSFER OUT %d / %d", p_event->data.eptransfer.ep, p_event->data.eptransfer.status);
        if (p_event->data.eptransfer.status == NRF_USBD_EP_WAITING) {
            struct usbd_ep_event *ev = usbd_evt_alloc();
            ev->ep = ep_ctx;
            ev->evt = EP_EVT_RECV_REQ;
            ...
            ep_ctx->buf.len = nrf_usbd_ep_amount_get(p_event->data.eptransfer.ep);
...
static void usbd_event_handler(nrfx_usbd_evt_t const * const p_event)
{
    ...
    case NRFX_USBD_EVT_SETUP: {
            ...
            if (drv_setup.bmRequest != USB_BMREQUEST_SETADDRESS) {
                ...
                struct nrf5_usbd_ep_ctx * ep_ctx = endpoint_ctx(NRF_USBD_EPOUT(0));
                ev = usbd_evt_alloc();
                ev->ep = ep_ctx;
		    ...
		    const u32_t data_len, u32_t * const ret_bytes)
{
	...
	// USBD hardware does not allow scheduling multiple DMA transfers at a time.
	// Next USB transfer can be triggered after all DMA operations are complete.
	if (sem_status != 0) {
	    SYS_LOG_DBGX("[%08X] !sem wr",(uint32_t)k_current_get());
		return -EAGAIN;
	}
	...
	if (err != NRFX_SUCCESS) {
	    SYS_LOG_DBGX(" --sem");
	    k_sem_give(&ctx->dma_in_use);
	    SYS_LOG_ERR("nRF USBD write error: %d.", (uint32_t)err);
	    __ASSERT_NO_MSG(0);
	}
